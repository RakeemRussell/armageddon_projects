...
```s
logging.basicConfig(...) > initializes Pythons logging system, states how the application should record logs

filename="/var/log/rdsapp.log" > tells Python "Write every log message into this file.", /var/log is the standard location for application and system logs

level=logging.INFO > setting level=logging.INFO, python records info, warning, error and critical but ignores debug messages
DEBUG   :Detailed troubleshooting           :Developers
INFO    :Normal application events          :Everyday operation
WARNING :Something unexpected but not fatal :Minor issues
ERROR   :An operation failed                :Database unavailable
CRITICAL:The application may stop working   :Major outage


format="%(asctime)s %(levelname)s %(message)s" > tells Python what every log line should look like
%(asctime)s  : Adds the current date and time
%(levelname)s: Adds the log level
%(message)s  : the message that is written in the code, Example: Database connection failed
...
```s
creates a logger object for current module, writes messages to the logger object according to the configuration that is already defined. __name__ automatically associates log messages with the module that generated them, making it easier to trace issues in larger applications and integrate with centralized logging systems like CloudWatch
...
> updated to code block below (changed db = c.get("dbname", "labdb") to db = c["dbname"]) (wrapped return pymysql.connect() with try:, except exception: logger.exception("Database connection failed") raise
...
try:
- tells python to attempt the code block because it might fail

except Exception 
- except: If something inside the try block fails, do not crash, do this first. Exception: Catch any exception that occurs while connecting to the database

raise
- tells Python, problem logged and continue to raise the original exception
```
